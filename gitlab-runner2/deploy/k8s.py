...
try:
    ...
    print("no MODE in myDeploy.py")
    mode = "MERGE"


if mode == "TRUNC":
    # 전부 삭제하고 다시 배포하므로, 삭제를 뿌린다
    deployf.write("sudo rancher kubectl get deployment -n hpc | awk '/"+group+"-"+project+"/ {print \"sudo rancher kubectl delete deployment \"$1\" -n "+group+" --wait=false\"}' > delete.sh\n")
