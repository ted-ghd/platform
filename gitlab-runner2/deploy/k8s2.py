...
pvcData = originPvcData
pvcData = pvcData.replace("#VOL_NAME#", "timezone")
pvcData = pvcData.replace("#HOST_PATH#", "/usr/share/zoneinfo/Asia/Seoul")
...
for i in range(0, int(pvc_cnt)):
    ...
    pvcData = pvcData.replace("#SC_NAME#", "rnd-rnd")
...
#################### PREPARE CONTAINER TEMPLATE ###############################
    ...
    for i in range(0, int(con_cnt)):
        ...
        # 당장 사용할 port 이외에 추가로 사용할 host port가 있는지 확인한다
        try:
            extra_int_port = dProps["EXTRA_HOST_INT_PORT"+str(i)]
            extra_ext_port = dProps["EXTRA_HOST_EXT_PORT"+str(i)]
            extra_int_port_str = ""
            extra_int_port_list = extra_int_port.split(" ")
            extra_ext_port_str = ""
            extra_ext_port_list = extra_ext_port.split(" ")

            extra_host_port_str=""

            idx = 0

            for extra_port_list_item in extra_int_port_list:
                extra_host_port_str = extra_host_port_str + "        - containerPort: "+str(extra_port_list_item)+"\n"\
                                + "          hostPort: "+str(extra_ext_port_list[idx])+"\n"

                idx = idx+1
            conData = conData.replace("#EXTRA_HOST_PORT#", extra_host_port_str)
        except KeyError as error:
            # extra_port가 없으면 con_data의 EXTRA_PORT를 지운다
            conData = conData.replace("#EXTRA_HOST_PORT#", "")    


        # 당장 사용할 port 이외에 추가로 사용할 port가 있는지 확인한다
        try:
            extra_port = dProps["EXTRA_PORT"+str(i)]
            extra_port_str = ""
            extra_port_list = extra_port.split(" ")
            for extra_port_list_item in extra_port_list:
                extra_port_str = extra_port_str + "        - containerPort: "+str(extra_port_list_item)+"\n"
            conData = conData.replace("#EXTRA_PORT#", extra_port_str)
        except KeyError as error:
            # extra_port가 없으면 con_data의 EXTRA_PORT를 지운다
            conData = conData.replace("#EXTRA_PORT#", "")    
        ...
        # KST 처리를 위해 timezone volume을 붙인다 
        conVolData = originConVolData
        conVolData = conVolData.replace("#CON_VOL_NAME#", "timezone")
        conVolData = conVolData.replace("#CON_VOL_PATH#", "/etc/localtime")
        merge_conVolData = merge_conVolData + conVolData
        
	# UTC를 KST로 표현하기 위해 KST volume을 마운트 시킨다
    ...
    for i in range(0, int(con_cnt)):
        # extra port가 선언되어 있는 경우에는 또 추가해서 붙여야 한다
        try:
            extra_port = dProps["EXTRA_PORT"+str(i)]
            extra_port_str = ""
            extra_port_list = extra_port.split(" ")
            for extra_port_list_item in extra_port_list:
                svcPortData = originSvcPortData
                svcPortData = svcPortData.replace("#CON_PORT#", str(extra_port_list_item))
                merge_svcData = merge_svcData + svcPortData
        except KeyError as error:
            # extra_port가 없으면 아무것도 할 필요가 없다
            merge_svcData = merge_svcData
...
for i in range(0, int(pvc_cnt)):
    ...
    try:
        ...
        deployf.write("rancher kubectl apply -f /temp/deploy/claim"+str(i)+".yaml\n");
    except KeyError as error:
...
for m in range(0, mul_dep):
    deployf.write("rancher kubectl "+mode_command+" --record -f /temp/deploy/deployment.yaml"+str(m)+"\n");
    
    # size가 0인 경우 제출하지 않는다
    if os.path.getsize("/temp/deploy/service.yaml"+str(m)) != 0:
        deployf.write("rancher kubectl apply -f /temp/deploy/service.yaml"+str(m)+"\n")
    ...
    for i in range(0, int(con_cnt)):
        ...
        if dProps["CON_PORT"+str(i)] == "22":
            continue
        deployf.write("rancher kubectl apply -f /temp/deploy/ingress"+str(i)+".yaml"+str(m)+"\n")
...
deployf.write("rancher kubectl get service --namespace="+group+"\n\n")
deployf.write("echo ============INGRESSES==============\n")
#deployf.write("sudo rancher kubectl get ingress --namespace="+group+"\n\n")
deployf.write("rancher kubectl get ingress --namespace="+group+" | awk '{ printf \"%-40s %-2s %-50s\\n\", $1, \" \", $2}'\n")
