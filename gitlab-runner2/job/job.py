...
dProps = load_properties("myJob.py");
 


# myJob.py에 ENABLE_CD값이 없거나, FALSE이면 Deploy 즉시 종료 하도록 한다
try:
    enable_cd = dProps['ENABLE_CD']
except KeyError as error:
    deployf = open("/temp/job/k8s_deploy_script", "w")
    deployf.write("echo Continuous Deployment is disabled. See myJob.ENABLE_CD");
    deployf.close()
    exit(0)
...
#wf=open("/temp/job/deployment.yaml", "w");
...
for i in range(0, int(pvc_cnt)):
    pvcF = open("/workdir/job/templates/pvc.tmp", "r")
    ...
    try:
        ...
        if pvcType == "HOST":
            ...
            hostPvcF = open("/workdir/job/templates/host_pvc.tmp", "r")
            originPvcData = hostPvcF.read()
            hostPvcF.close() 
    except KeyError as error:
        # Type이 없으면 NAS PVC 생성으로 간주한다
        print("no "+str(error)+"in myJob.py")
...
try:
    ...
    shmF = open("/workdir/job/templates/shm.tmp", "r")
    shmData = shmF.read()
    shmF.close()
...
# myJob.py에 작성된 pvc volume을 k8s에 제줄할 yaml을 만든다
for i in range(0, int(pvc_cnt)):
    claimF=open("/temp/job/claim"+str(i)+".yaml", "w");
    
    pvcF = open("/workdir/job/templates/claim.tmp", "r")
    originPvcData = pvcF.read()
    pvcF.close()
...
try:
    ...
    # 경고메세지만 출력하고 계속한다
    print("no MUL_DEP option in myJob.py")
...
#################### PREPARE CONTAINER TEMPLATE ###############################
    wf=open("/temp/job/deployment.yaml"+str(m), "w");
    conF = open("/workdir/job/templates/container.tmp", "r")
    ...
    try:
        ...
        print("no CON_CNT option in myJob.py")
        exit(1)
    ...
    for i in range(0, int(con_cnt)):
        ...
        #    except KeyError as error:
                ...
                # 경고메세지를 출력하고 종료 시킨다
        #        print("no EXP_PORT option in myJob.py")
        ...
        conVolF = open("/workdir/job/templates/container_volume.tmp","r")
        ...
        gpuF = open("/workdir/job/templates/gpus.tmp", "r")
        gpuData = gpuF.read()
        gpuF.close()
    ...
    dep_type=""	
    try:
        dep_type=dProps['DEP_TYPE']
    except KeyError as error:
        print("no "+str(error)+" in myJob.py. Consider as DEPLOYMENT")
        dep_type="Deploy"

        # job template를 불러온다
    if dep_type == "JOB":
        depF = open("/workdir/job/templates/job.tmp", "r")
    else:
        # deployment template을 불러온다
        depF = open("/workdir/job/templates/deployment.tmp", "r")
    ...
    #wf=open("/temp/job/service.yaml"+str(m), "w");

    #svcPortF = open("/workdir/job/templates/service_port.tmp", "r")
    ...
    #svcF = open("/workdir/job/templates/service.tmp", "r")
    #svcData = svcF.read()
    #svcF.close()
    ...
    #    wf=open("/temp/job/ingress"+str(i)+".yaml"+str(m), "w");
    #    ingressF = open("/workdir/job/templates/ingress.tmp", "r")
    ...
    #wf=open("/temp/job/namespace.yaml"+str(m), "w");
    #nsF = open("/workdir/job/templates/namespace.tmp", "r")
    #nsData = nsF.read()
    #nsF.close()
...
for i in range(0, int(pvc_cnt)):
    ...
    try:
        ...
        deployf.write("sudo rancher kubectl apply -f /temp/job/claim"+str(i)+".yaml\n");
    except KeyError as error:
...
for m in range(0, mul_dep):
    deployf.write("sudo rancher kubectl "+mode_command+" --record -f /temp/job/deployment.yaml"+str(m)+"\n");
    
    # size가 0인 경우 제출하지 않는다
    #if os.path.getsize("/temp/job/service.yaml"+str(m)) != 0:
    #    deployf.write("sudo rancher kubectl apply -f /temp/job/service.yaml"+str(m)+"\n")
    ...
    #    if dProps["CON_PORT"+str(i)] == "22":
    #        continue
    #    deployf.write("sudo rancher kubectl apply -f /temp/job/ingress"+str(i)+".yaml"+str(m)+"\n")
